...
{
	[POCOViewModel]
	public class EmailChargeRunViewModel : ViewModelBase
	{
		#region Services
		...
		#endregion
		public virtual EmailCharge Entity { get; set; }
		public virtual ObservableCollection<EmailChargeRecipient> Recipients { get; set; }
		...
		public EmailChargeRunViewModel() : base()
		{
			MessengerHelper.Register<MsgRowChange<EmailChargeRecipient>>(this, OnMsgRowChange);
		}
		...
		public void Submit()
		{
			DispatcherUIHelper.Run(async () =>
			{
				...
				updateEntity.EmailChargeRecipients = Recipients.Where(q => q.IsChecked && !string.IsNullOrEmpty(q.Email)).ToList();
				updateEntity.EmailChargeAttachments = OpenParam.Attachments.ToList();

				//save
				ShowWaitIndicator.Show(ShowWaitIndicator.Mode.Custom, "Sending email...");
				var uret = await businessService.SendChargeEmail(updateEntity);
		...
		void OnMsgRowChange(MsgRowChange<EmailChargeRecipient> msg)
		{
		...
		async Task LoadData()
		{
			...
			{
				WindowTitle = "Send Chargeout by Email";
			}
			else throw new ArgumentException();

			Entity = new EmailCharge
			{
				ChargeoutRowId = OpenParam.ChargeoutRowId,
			};
		...
		public class OpenParams
		{
			...
			public IEnumerable<EmailChargeRecipient> Recipients { get; set; }
			public IEnumerable<EmailChargeAttachment> Attachments { get; set; }
		...
		public static Task<ReturnParams> Run(OpenParams parm)
		{
			...
			{
				ViewCode = ViewCodes.EmailChargeRunView,
				Param = parm,
			});
