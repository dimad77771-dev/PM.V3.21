...
{
	public class AppointmentsSchedulerToolTipBehavior : Behavior<ToolTip>
	{
		...
		public static readonly DependencyProperty SourceProperty =
			DependencyProperty.Register("Source", typeof(object), typeof(AppointmentsSchedulerToolTipBehavior));
		...
		private void OnOpened(object sender, RoutedEventArgs e)
		{
			...
			var viewmodel = LayoutTreeHelper.GetVisualParents(AssociatedObject.PlacementTarget)
				...
				.Select(q => q.DataContext as AppointmentsSchedulerViewModel)
				.First(q => q != null);

			if (!viewmodel.IsOnePatientOneAppointment)
			{
				AssociatedObject.Visibility = Visibility.Hidden;
				return;
			}
			...
			var appointment = viewmodel.FindInIntervalAllAppointmentInAppointmentBook(iStart, iFinish).FirstOrDefault();

			if (appointment != null)
			{
				var model = new VisualAppointmentViewInfo
				{
					CustomViewInfo = viewmodel.Appointment2HospitalAppointment(appointment),
				};
				...
				var border = allChilds.Single(q => q.Name == "borderAppointmentTooltip") as Border;
