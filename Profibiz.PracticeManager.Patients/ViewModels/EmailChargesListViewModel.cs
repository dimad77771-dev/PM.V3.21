...
{
	[POCOViewModel]
	public class EmailChargesListViewModel : ViewModelBase
	{
		#region Services
		...
		#endregion
		public virtual ObservableCollection<EmailCharge> Entities { get; set; }
		public virtual EmailCharge SelectedEntity { get; set; }
		public virtual DateTime FilterFrom { get; set; }
		public virtual DateTime FilterTo { get; set; }

		~EmailChargesListViewModel()
		{
			NLog.Trace("~EmailChargesListViewModel=" + this.GetHashCode());
		}


		public EmailChargesListViewModel() : base()
		{
			NLog.vv(() => "EmailChargesListViewModel.create=" + this.GetHashCode());
			var ret = GlobalSettings.Instance.EmailChargeList.Get();
		...
		public void OnOpen(string parm)
		{
			...
			//MessengerHelper.Register<MsgRowChange<EmailCharge>>(this, OnMsgRowChange);
		...
		public async Task LoadData()
		{
			...
			var rows = await lookupsBusinessService.RunTaskAndUpdateAllLookups(businessService.GetEmailChargeList(query));
		...
		void SubscribeListRow(EmailCharge row)
		{
			row.OnOpenAttachment = () =>
			{
				DispatcherUIHelper.Run(async () =>
				{
					...
					var attachments = await businessService.GetEmailChargeAttachmentList(query);
					ShowWaitIndicator.Hide();
		...
		public void FilterCore(string arg = "", FinanceDateClass preset = null)
		{
			DispatcherUIHelper.Run(async () =>
			{
				...
				GlobalSettings.Instance.EmailChargeList.Set(FilterFrom, FilterTo);
				await LoadData();
			});
		}
	}
