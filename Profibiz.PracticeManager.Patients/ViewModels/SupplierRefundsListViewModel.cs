...
{
	[POCOViewModel]
	public class SupplierRefundsListViewModel : ViewModelBase
	{
		#region Service
		...
		#endregion
		public virtual ObservableCollection<SupplierRefund> Entities { get; set; }
		public virtual SupplierRefund SelectedEntity { get; set; }
		public virtual SupplierRefundOneViewModel OneModel { get; set; }
		...
		public SupplierRefundsListViewModel() : base()
		{
		...
		public void OnOpen(string parm)
		{
			...
			MessengerHelper.Register<MsgRowChange<SupplierRefund>>(this, OnMsgRowChange);
		...
		async Task LoadData()
		{
			...
			var task = BusinessServiceHelprer.GetSupplierRefundList(paymentDateFrom: FilterFrom, paymentDateTo: FilterTo);
		...
		void OnMsgRowChange(MsgRowChange<SupplierRefund> msg)
		{
			MessengerHelper.RunAction(this, async () =>
			{
				ShowWaitIndicator.Show();
				var SupplierRefundRowId = msg.Row.RowId;
				var rows = await lookupsBusinessService.RunTaskAndUpdateAllLookups(businessService.GetSupplierRefundList("rowId=" + SupplierRefundRowId));
				if (rows.Count > 0)
				{
					var row = rows[0];
					var index = Entities.FindIndex(q => q.RowId == SupplierRefundRowId);
					if (index >= 0)
					{
						Entities[index] = row;
					}
		...
		public void OnSelectedEntityChanged(SupplierRefund oldrow)
		{
			DispatcherUIHelper.Run(async () =>
			{
		...
		SupplierRefundOneViewModel CreateOneModel()
		{
			return SupplierRefundOneViewModel.Create(isWindowMode: false);
		}
		...
		//			IsNew = true,
		//		};
		//		var nrow = new SupplierRefund
		//		{
		//			RowId = SupplierRefund.RowId,
		//			PatientFullName = patient.FullName,
		//			PatientRowId = patient.RowId,
		//			SupplierPaymentDate = SupplierRefund.SupplierPaymentDate,
		//			IsNew = true,
		//		};

		//		OneModel = CreateOneModel();
		//		await OneModel.LoadData(newSupplierRefund: SupplierRefund);
		...
		public async void Delete()
		{
			...
			var ret = messageBoxService.ShowMessage(string.Format(CommonResources.Confirmation_Delete, "SupplierRefund"), CommonResources.Confirmation_Caption, MessageButton.YesNo, MessageIcon.Question);
			if (ret == MessageResult.Yes)
			{
				var row = SelectedEntity;
				ShowWaitIndicator.Show(ShowWaitIndicator.Mode.Save);
				var uret = await businessService.DeleteSupplierRefund(row.RowId);
				ShowWaitIndicator.Hide();
