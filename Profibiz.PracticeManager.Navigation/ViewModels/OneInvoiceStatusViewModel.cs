...
{
	[POCOViewModel]
	public class OneInvoiceStatusViewModel : ViewModelBase
	{
		#region Services
		...
		public virtual InvoiceStatus Entity { get; set; }
		public bool IsNew { get; set; }


		public OneInvoiceStatusViewModel(IPatientsBusinessService _businessService, ILookupsBusinessService _lookupsBusinessService) : base()
		{
		...
		async void LoadData()
		{
			...
			if (!IsNew)
			{
				entity = LookupDataProvider.FindInvoiceStatus(OpenParam.RowId).GetPocoClone();
			}
			else
			{
		...
		async Task<bool> SaveCore(bool andClose)
		{
			...
			var uret = await lookupsBusinessService.PutInvoiceStatuses(updateEntities);
			...
			MessengerHelper.Send(new MsgRowLookupUpdate(MsgRowLookupUpdate.TableEnum.InvoiceStatuses));
