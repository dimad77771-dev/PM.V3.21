...
{
	[POCOViewModel]
	public class OneSupplierViewModel : ViewModelBase
	{
		#region Services
		...
		public OneSupplierViewModel(IPatientsBusinessService _businessService, ILookupsBusinessService _lookupsBusinessService) : base()
		{
		...
		public void OnOpen(object arg)
		{
			...
			else if (arg is String)
			{
				...
				else if (((string)arg).StartsWith("OpenRowId="))
				{
					OpenParam = new OpenParams { IsNew = false, RowId = Guid.Parse(((string)arg).Replace("OpenRowId=", "")) };
				}
			}
			LoadData();
		}
		...
		async void LoadData()
		{
			...
			if (!IsNew)
			{
				entity = LookupDataProvider.FindSupplier(OpenParam.RowId).GetPocoClone();
			}
			else
			{
				entity = new Supplier();
		...
		async Task<bool> SaveCore(bool andClose)
		{
			...
			var uret = await lookupsBusinessService.PutSuppliers(updateEntities);
			...
			MessengerHelper.Send(new MsgRowLookupUpdate(MsgRowLookupUpdate.TableEnum.Suppliers));
