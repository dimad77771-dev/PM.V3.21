...
namespace Profibiz.PracticeManager.Navigation.ViewModels
{
	...
	public class CategoriesViewModel : ViewModelBase
	{
		...
		public virtual ObservableCollection<Category> Entities { get; set; }
		public virtual Category SelectedEntity { get; set; }
		...
		public CategoriesViewModel(IPatientsBusinessService _businessService, ILookupsBusinessService _lookupsBusinessService) : base()
		{
		...
		async void LoadData()
		{
			...
			Entities = LookupDataProvider.Instance.Categories.OrderBy(q => q.Name).ToObservableCollection();
		...
		public async void Delete()
		{
			...
			var ret = messageBoxService.ShowMessage(string.Format(CommonResources.Confirmation_Delete, "Category"), CommonResources.Confirmation_Caption, MessageButton.YesNo, MessageIcon.Question);
			if (ret == MessageResult.Yes)
			{
				ShowWaitIndicator.Show(ShowWaitIndicator.Mode.Save);
				var uret = await lookupsBusinessService.DeleteCategory(row);
				ShowWaitIndicator.Hide();
		...
		void AddEdit(Category row)
		{
			ShowDXWindowsInteractionRequest.Raise(new ShowDXWindowsActionParam
			{
				ViewCode = ViewCodes.OneCategoryView,
				Param = new OneCategoryViewModel.OpenParams { IsNew = (row == null), RowId = (row == null ? default(Guid) : row.RowId) },
			});
		}
