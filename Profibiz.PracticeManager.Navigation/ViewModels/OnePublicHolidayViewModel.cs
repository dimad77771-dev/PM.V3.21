...
{
	[POCOViewModel]
	public class OnePublicHolidayViewModel : ViewModelBase
	{
		#region Services
		...
		public virtual PublicHoliday Entity { get; set; }
		public bool IsNew { get; set; }


		public OnePublicHolidayViewModel(IPatientsBusinessService _businessService, ILookupsBusinessService _lookupsBusinessService) : base()
		{
		...
		async void LoadData()
		{
			...
			if (!IsNew)
			{
				entity = LookupDataProvider.FindPublicHoliday(OpenParam.RowId).GetPocoClone();
			}
			else
			{
				entity = new PublicHoliday();
				entity.RowId = Guid.NewGuid();
				entity.HolidayDate = new DateTime(DateTime.Today.Year, 1, 1);
			}
			Entity = entity;
			ResetHasChange();
		...
		async Task<bool> SaveCore(bool andClose)
		{
			...
			var uret = await lookupsBusinessService.PutPublicHolidays(updateEntities);
			...
			MessengerHelper.Send(new MsgRowLookupUpdate(MsgRowLookupUpdate.TableEnum.PublicHolidays));
