...
{
	[POCOViewModel]
	public class OneCalendarEventStatusViewModel : ViewModelBase
	{
		#region Services
		...
		public virtual CalendarEventStatus Entity { get; set; }
		public bool IsNew { get; set; }


		public OneCalendarEventStatusViewModel(IPatientsBusinessService _businessService, ILookupsBusinessService _lookupsBusinessService) : base()
		{
		...
		async void LoadData()
		{
			...
			CalendarEventStatus entity;
			if (!IsNew)
			{
				entity = LookupDataProvider.FindCalendarEventStatus(OpenParam.RowId).GetPocoClone();
			}
			else
			{
				entity = new CalendarEventStatus();
		...
		async Task<bool> SaveCore(bool andClose)
		{
			...
			var uret = await lookupsBusinessService.PutCalendarEventStatuses(updateEntities);
			...
			MessengerHelper.Send(new MsgRowLookupUpdate(MsgRowLookupUpdate.TableEnum.CalendarEventStatuses));
