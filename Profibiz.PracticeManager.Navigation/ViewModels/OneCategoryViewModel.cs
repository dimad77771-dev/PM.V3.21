...
{
	[POCOViewModel]
	public class OneCategoryViewModel : ViewModelBase
	{
		#region Services
		...
		public OneCategoryViewModel(IPatientsBusinessService _businessService, ILookupsBusinessService _lookupsBusinessService) : base()
		{
		...
		public void OnOpen(object arg)
		{
			...
			else if (arg is String)
			{
                ...
                {
                    OpenParam = new OpenParams { IsNew = true, NewItemType = TypeHelper.CategoryType.Supply };
                }
                else if ((string)arg == "IsNew;ItemType=Service")
                {
                    OpenParam = new OpenParams { IsNew = true, NewItemType = TypeHelper.CategoryType.Service };
                }
			}
			LoadData();
		}
		...
		async void LoadData()
		{
			...
			else
			{
                ...
                {
                    entity.CategoryType = OpenParam.NewItemType;
                }
            }
			Entity = entity;
			ResetHasChange();
		...
		bool Validate()
		{
			...
			ValidateHelper.Empty(Entity.CategoryType, "Type", errors);
		...
		async Task<bool> SaveCore(bool andClose)
		{
			...
			var uret = await lookupsBusinessService.PutCategories(updateEntities);
			...
			MessengerHelper.Send(new MsgRowLookupUpdate(MsgRowLookupUpdate.TableEnum.Categories));
