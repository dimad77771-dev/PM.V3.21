...
{
	[POCOViewModel]
	public class OneAppointmentStatusViewModel : ViewModelBase
	{
		#region Services
		...
		public virtual AppointmentStatus Entity { get; set; }
		public bool IsNew { get; set; }


		public OneAppointmentStatusViewModel(IPatientsBusinessService _businessService, ILookupsBusinessService _lookupsBusinessService) : base()
		{
		...
		async void LoadData()
		{
			...
			if (!IsNew)
			{
				entity = LookupDataProvider.FindAppointmentStatus(OpenParam.RowId).GetPocoClone();
			}
			else
			{
				entity = new AppointmentStatus();
		...
		async Task<bool> SaveCore(bool andClose)
		{
			...
			var uret = await lookupsBusinessService.PutAppointmentStatuses(updateEntities);
			...
			MessengerHelper.Send(new MsgRowLookupUpdate(MsgRowLookupUpdate.TableEnum.AppointmentStatuses));
