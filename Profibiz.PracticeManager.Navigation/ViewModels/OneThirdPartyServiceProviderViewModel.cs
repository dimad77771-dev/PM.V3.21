...
{
	[POCOViewModel]
	public class OneThirdPartyServiceProviderViewModel : ViewModelBase
	{
		#region Services
		...
		public virtual ThirdPartyServiceProvider Entity { get; set; }
		public bool IsNew { get; set; }


		public OneThirdPartyServiceProviderViewModel(IPatientsBusinessService _businessService, ILookupsBusinessService _lookupsBusinessService) : base()
		{
		...
		async void LoadData()
		{
			...
			ThirdPartyServiceProvider entity;
			if (!IsNew)
			{
				entity = LookupDataProvider.FindThirdPartyServiceProvider(OpenParam.RowId).GetPocoClone();
			}
			else
			{
				entity = new ThirdPartyServiceProvider();
		...
		async Task<bool> SaveCore(bool andClose)
		{
			...
			var uret = await lookupsBusinessService.PutThirdPartyServiceProviders(updateEntities);
			...
			MessengerHelper.Send(new MsgRowLookupUpdate(MsgRowLookupUpdate.TableEnum.ThirdPartyServiceProviders));
