...
{
	[POCOViewModel]
	public class ProfessionalAssociationsViewModel : ViewModelBase
	{
		#region Services
		...
		public virtual ObservableCollection<ProfessionalAssociation> Entities { get; set; }
		public virtual ProfessionalAssociation SelectedEntity { get; set; }
		...
		public ProfessionalAssociationsViewModel(IPatientsBusinessService _businessService, ILookupsBusinessService _lookupsBusinessService) : base()
		{
		...
		public void OnOpen(string parm)
		{
			...
			LoadData();
			MessengerHelper.Register<MsgRowChange<ProfessionalAssociation>>(this, OnMsgRowChange);
		}
		...
		async void LoadData()
		{
			...
			Entities = LookupDataProvider.Instance.ProfessionalAssociations.OrderBy(q => q.Name).ToObservableCollection();
			ShowWaitIndicator.Hide();
			DXSplashScreenHelper.Hide();
		}

		void OnMsgRowChange(MsgRowChange<ProfessionalAssociation> msg)
		{
		...
		void AddEdit(ProfessionalAssociation row)
		{
			ShowDXWindowsInteractionRequest.Raise(new ShowDXWindowsActionParam
			{
				ViewCode = ViewCodes.OneProfessionalAssociationView,
				Param = new OneProfessionalAssociationViewModel.OpenParams { IsNew = (row == null), RowId = (row == null ? default(Guid) : row.RowId) },
			});
		}
