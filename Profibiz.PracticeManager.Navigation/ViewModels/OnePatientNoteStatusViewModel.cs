...
{
	[POCOViewModel]
	public class OnePatientNoteStatusViewModel : ViewModelBase
	{
		#region Services
		...
		public virtual PatientNoteStatus Entity { get; set; }
		public bool IsNew { get; set; }


		public OnePatientNoteStatusViewModel(IPatientsBusinessService _businessService, ILookupsBusinessService _lookupsBusinessService) : base()
		{
		...
		async void LoadData()
		{
			...
			PatientNoteStatus entity;
			if (!IsNew)
			{
				entity = LookupDataProvider.FindPatientNoteStatus(OpenParam.RowId).GetPocoClone();
			}
			else
			{
				entity = new PatientNoteStatus();
		...
		async Task<bool> SaveCore(bool andClose)
		{
			...
			var uret = await lookupsBusinessService.PutPatientNoteStatuses(updateEntities);
			...
			MessengerHelper.Send(new MsgRowLookupUpdate(MsgRowLookupUpdate.TableEnum.PatientNoteStatuses));
