...
{
	[POCOViewModel]
	public class OneReferrerViewModel : ViewModelBase
	{
		#region Services
		...
		public virtual Referrer Entity { get; set; }
		public bool IsNew { get; set; }


		public OneReferrerViewModel(IPatientsBusinessService _businessService, ILookupsBusinessService _lookupsBusinessService) : base()
		{
		...
		async void LoadData()
		{
			...
			if (!IsNew)
			{
				entity = LookupDataProvider.FindReferrer(OpenParam.RowId).GetPocoClone();
			}
			else
			{
				entity = new Referrer();
		...
		async Task<bool> SaveCore(bool andClose)
		{
			...
			var uret = await lookupsBusinessService.PutReferrers(updateEntities);
			...
			MessengerHelper.Send(new MsgRowLookupUpdate(MsgRowLookupUpdate.TableEnum.Referrers));
