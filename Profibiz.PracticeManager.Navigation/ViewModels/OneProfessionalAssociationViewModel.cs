...
{
	[POCOViewModel]
	public class OneProfessionalAssociationViewModel : ViewModelBase
	{
		#region Services
		...
		public virtual ProfessionalAssociation Entity { get; set; }
		public bool IsNew { get; set; }


		public OneProfessionalAssociationViewModel(IPatientsBusinessService _businessService, ILookupsBusinessService _lookupsBusinessService) : base()
		{
		...
		async void LoadData()
		{
			...
			if (!IsNew)
			{
				entity = LookupDataProvider.FindProfessionalAssociation(OpenParam.RowId).GetPocoClone();
			}
			else
			{
				entity = new ProfessionalAssociation();
		...
		async Task<bool> SaveCore(bool andClose)
		{
			...
			var uret = await lookupsBusinessService.PutProfessionalAssociations(updateEntities);
			...
			MessengerHelper.Send(new MsgRowLookupUpdate(MsgRowLookupUpdate.TableEnum.ProfessionalAssociations));
