...
{
	[POCOViewModel]
	public class OneAppointmentBookViewModel : ViewModelBase
	{
		#region Services
		...
		public virtual AppointmentBook Entity { get; set; }
		public bool IsNew { get; set; }


		public OneAppointmentBookViewModel(IPatientsBusinessService _businessService, ILookupsBusinessService _lookupsBusinessService) : base()
		{
		...
		public void OnOpen(object arg)
		{
			...
			else if (arg is String)
			{
				...
				{
					var pm = QueryHelper.ParseString(s);
		...
		async void LoadData()
		{
			...
			if (!IsNew)
			{
				entity = LookupDataProvider.FindAppointmentBook(OpenParam.RowId).GetPocoClone();
			}
			else
			{
				entity = new AppointmentBook();
				entity.RowId = Guid.NewGuid();
				entity.Name = OpenParam.NewName;
			}
			Entity = entity;
			ResetHasChange();
		...
		bool Validate()
		{
			...
			ValidateHelper.Positive(Entity.Interval, "Interval", errors);
			var time1 = Entity.StartAt.TimeOfDay;
			var time2 = Entity.FinishAt.TimeOfDay;
			if (time2 <= time1)
			{
				errors.Add("\"Finish At\" must be more then \"Start At\"");
			}

			Entity.StartAt = new DateTime(1900, 1, 1) + time1;
			Entity.FinishAt = new DateTime(1900, 1, 1) + time2;
		...
		async Task<bool> SaveCore(bool andClose)
		{
			...
			var uret = await lookupsBusinessService.PutAppointmentBooks(updateEntities);
			...
			MessengerHelper.Send(new MsgRowLookupUpdate(MsgRowLookupUpdate.TableEnum.AppointmentBooks));
