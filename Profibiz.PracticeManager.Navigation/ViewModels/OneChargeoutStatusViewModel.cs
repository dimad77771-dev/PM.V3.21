...
{
	[POCOViewModel]
	public class OneChargeoutStatusViewModel : ViewModelBase
	{
		#region Services
		...
		public virtual ChargeoutStatus Entity { get; set; }
		public bool IsNew { get; set; }


		public OneChargeoutStatusViewModel(IPatientsBusinessService _businessService, ILookupsBusinessService _lookupsBusinessService) : base()
		{
		...
		async void LoadData()
		{
			...
			ChargeoutStatus entity;
			if (!IsNew)
			{
				entity = LookupDataProvider.FindChargeoutStatus(OpenParam.RowId).GetPocoClone();
			}
			else
			{
				entity = new ChargeoutStatus();
		...
		async Task<bool> SaveCore(bool andClose)
		{
			...
			var uret = await lookupsBusinessService.PutChargeoutStatuses(updateEntities);
			...
			MessengerHelper.Send(new MsgRowLookupUpdate(MsgRowLookupUpdate.TableEnum.ChargeoutStatuses));
