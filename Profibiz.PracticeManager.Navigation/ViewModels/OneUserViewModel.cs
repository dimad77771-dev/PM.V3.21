...
{
	[POCOViewModel]
	public class OneUserViewModel : ViewModelBase
	{
		#region Services
		...
		public OneUserViewModel(IPatientsBusinessService _businessService, ILookupsBusinessService _lookupsBusinessService) : base()
		{
		...
		async void LoadData()
		{
			...
			if (!IsNew)
			{
				entity = LookupDataProvider.FindUser(OpenParam.RowId).GetPocoClone();
			}
			else
			{
				entity = new User();
		...
		async Task<bool> SaveCore(bool andClose)
		{
			...
			var uret = await lookupsBusinessService.PutUsers(updateEntities);
			...
			MessengerHelper.Send(new MsgRowLookupUpdate(MsgRowLookupUpdate.TableEnum.Users));
