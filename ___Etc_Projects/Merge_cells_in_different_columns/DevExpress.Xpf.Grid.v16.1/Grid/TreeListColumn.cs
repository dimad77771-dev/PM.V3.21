// Decompiled with JetBrains decompiler
// Type: DevExpress.Xpf.Grid.TreeListColumn
// Assembly: DevExpress.Xpf.Grid.v16.1, Version=16.1.6.0, Culture=neutral, PublicKeyToken=b88d1754d700e49a
// MVID: BB681040-0474-4C7D-BB3E-E6E5DFDDD1F8
// Assembly location: H:\DOWNLOADS\T145231\T145231\bin\Debug\DevExpress.Xpf.Grid.v16.1.dll

using DevExpress.Xpf.Grid.TreeList;

namespace DevExpress.Xpf.Grid
{
  /// <summary>
  ///                 <para>A column within a <see cref="T:DevExpress.Xpf.Grid.TreeListControl" />.
  /// 
  /// </para>
  ///             </summary>
  public class TreeListColumn : GridColumnBase
  {
    /// <summary>
    ///                 <para>Enables you to validate the active editor's value.
    /// </para>
    ///             </summary>
    public event TreeListCellValidationEventHandler Validate;

    protected internal override void OnValidation(GridRowValidationEventArgs e)
    {
      ((TreeListColumn) this.GetEventTargetColumn()).OnValidationCore(e);
    }

    private void OnValidationCore(GridRowValidationEventArgs e)
    {
      if (this.Validate == null)
        return;
      this.Validate((object) this, (TreeListCellValidationEventArgs) e);
    }
  }
}
