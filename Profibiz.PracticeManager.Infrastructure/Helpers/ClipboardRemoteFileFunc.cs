...
{
	public static class ClipboardRemoteFileFunc
	{
		public static Retrun Run()
		{
			...
			{
				format2 = "FileGroupDescriptorW";
			}
			else if (formats.Contains("FileGroupDescriptor") && formats.Contains("FileContents"))
			{
				format2 = "FileGroupDescriptor";
			}

			if (!string.IsNullOrEmpty(format2))
			{
				var dataObject = new OutlookDataObject(Clipboard.GetDataObject());
				var files = (string[])dataObject.GetData("FileGroupDescriptorW", false);
				var fileContents = (MemoryStream[])dataObject.GetData("FileContents", false, files);
				var fileContentsBytes = fileContents.Select(q => q.ToArray()).ToArray();
				return new Retrun { Files = files, FileContentsBytes = fileContentsBytes };
			}
			else
			{
				return null;
		...
		public class Retrun
		{
			...
			public byte[][] FileContentsBytes { get; set; }
		}
		...
		public class OutlookDataObject : System.Windows.Forms.IDataObject
		{
			...
			{
				return this.GetData(format, autoConvert, null);
			}

			public object GetData(string format, bool autoConvert, string[] fileContentNames0)
			{
				switch (format)
				{
					...
					case "FileGroupDescriptorW":
						...
						try
						{
							...
							//MemoryStream fileGroupDescriptorStream = (MemoryStream)Clipboard.GetDataObject().GetData("FileGroupDescriptorW");
					...
					case "FileContents":
						...
						//string[] fileContentNames = (string[])this.GetData("FileGroupDescriptorW");
						string[] fileContentNames = fileContentNames0;
